...
- (void)installRemoteCommandsWithPlay:(dispatch_block_t)play pause:(dispatch_block_t)pause next:(dispatch_block_t)next previous:(dispatch_block_t)previous { MPRemoteCommandCenter *remote=MPRemoteCommandCenter.sharedCommandCenter; NSMutableArray *targets=[NSMutableArray array]; MPRemoteCommandHandlerStatus (^handler)(MPRemoteCommandEvent *,dispatch_block_t) = ^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent *event, dispatch_block_t block) { if (block) block(); return MPRemoteCommandHandlerStatusSuccess; }; if (play) [targets addObject:[remote.playCommand addTargetWithHandler:^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent *event){ return handler(event,play); }]]; if (pause) [targets addObject:[remote.pauseCommand addTargetWithHandler:^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent *event){ return handler(event,pause); }]]; if (next) [targets addObject:[remote.nextTrackCommand addTargetWithHandler:^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent *event){ return handler(event,next); }]]; if (previous) [targets addObject:[remote.previousTrackCommand addTargetWithHandler:^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent *event){ return handler(event,previous); }]]; self.targets=targets; }
